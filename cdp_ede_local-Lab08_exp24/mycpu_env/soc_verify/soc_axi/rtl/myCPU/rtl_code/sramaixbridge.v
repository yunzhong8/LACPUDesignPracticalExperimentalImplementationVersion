...
9. 写会外部RAM的写的顺序发错了，应该先发低字节的，而我写成先发高字节的数据了
\*************/
...
/***************************************parameter define(常量定义)**************************************/
  ...
  parameter WEmpty = 3'b000;//读初始状态 0
  
   parameter WAcceptCPUData = 3'b001;//读持续接受CPU数据` 1
   parameter WWaitExtAXIAcceptAddrData = 3'b010;//读持续接受CPU数据`2
   parameter WWaitExtAXIAcceptData = 3'b011;//读持续接受CPU数据`3
   parameter WWaitExtAXIAcceptAddr = 3'b100;//读持续接受CPU数据`4
   parameter WWaitExtAXIWriteFnish = 3'b101;//5
   reg[1:0]r_cs,r_ns;
   reg [2:0]w_cs,w_ns;
...
*/
 ...
 assign m_wdata = axi_w_data[wirte_back_index];
