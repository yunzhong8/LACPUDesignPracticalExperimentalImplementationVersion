...
;
 ...
 //完成发ce
  ...
  assign ridle_can_respone_ls       = rcs_eq_ridle      & (~cacop_en_i)&(~store_buffer_we_i);
  assign read_can_respone_ls        = rcs_eq_read       & (~cacop_en)&cpu_mmu_finish_i &(~cache_search_op);
  assign lookup_can_respone_ls      = rcs_eq_lookup     & lookup_req_finish_no_write_cache&(search_buffer_we_count&(~cache_wdata_buffer_valid) |~search_buffer_we_count);
  assign write_can_respone_ls       = rcs_eq_write      & write_req_finish_no_write_cache&(~search_buffer_valid);
  assign writeback_can_respone_ls   = rcs_eq_writeback  & writeback_req_finish_no_write_cache&(~search_buffer_valid);
  assign writeblock_can_respone_ls  = rcs_eq_writeblock & writeblock_req_finish_no_write_cache&(~search_buffer_valid);
  ...
  //请求读外界数据
   ...
   assign  axi_write_count_ce    =   rcs_eq_refill&ret_valid &ret_last;              
        ...
        assign cs_eq_lookup_ns = ( 
                                   lookup_req_finish_no_write_cache & ~search_buffer_we_count & (cache_re|cache_we)    
    ...
    assign cs_eq_writeback_ns = cacop_en ? WRITEBLOCK :
                                (   
                                    writeback_req_finish_no_write_cache&(~search_buffer_valid)&(cache_re|cache_we)
    ...
    assign cs_eq_writeblock_ns = writeblock_req_finish&search_buffer_valid &cpu_mmu_finish_i ? LOOKUP :
                                 (  
                                    writeblock_req_finish_no_write_cache&~search_buffer_valid&(cache_re|cache_we))
